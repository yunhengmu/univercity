<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="upLogList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="书目ID" align="center" prop="bookId" width="100" />
      ...
      <el-table-column label="上架时间" align="center" prop="createTime" width="160">
        <template slot-scope="scope">
    ...
    <!-- 查看详情对话框 -->
    <el-dialog title="上架记录详情" :visible.sync="viewOpen" width="600px" append-to-body>
...
<script>
import { listUpLog, listAllUpLog, getUpLog, delUpLog, batchDelUpLog, shelveUpLog, unshelveUpLog } from "@/api/upLog"
import { UpLogVO, UpLogQueryVO } from "@/vo/upLog"
...
export default {
  ...
  methods: {
    ...
    getUpLogVO(data) {
      ...
      return UpLogVO.fromObject(data)
    },

    /** 查询上架记录列表 */
    getList() {
      this.loading = true
      // 使用listAllUpLog获取所有上架记录(不带筛选条件)
      const params = {
      ...
      listAllUpLog(params).then(response => {
        ...
        console.warn('[UpLog] 获取列表失败,请检查后端接口是否可用:', error.message)
        this.upLogList = []
        this.total = 0
        this.loading = false
      })
    ...
    handleShelve(row) {
      const id = row.id
      const vo = this.getUpLogVO(row)

      this.$modal.confirm(`确认要上架书目ID为"${vo.bookId}"的记录吗？`).then(() => {
    ...
    handleUnshelve(row) {
      ...
      this.$modal.confirm(`确认要下架书目ID为"${vo.bookId}"的记录吗？`).then(() => {
    ...
    handleView(row) {
      const id = row.id
      getUpLog(id).then(response => {
        this.viewData = response.data
        this.viewDataVO = this.getUpLogVO(this.viewData)
        this.viewOpen = true
      })
    },
    ...
    handleDelete(row) {
      ...
      const vo = row.id ? this.getUpLogVO(row) : null

      this.$modal.confirm(`是否确认删除${vo ? `书目ID为"${vo.bookId}"的` : ''}上架记录编号为"${ids}"的数据项？`).then(() => {
        if (Array.isArray(ids) && ids.length > 1) {
          return batchDelUpLog(ids)
        } else {
          return delUpLog(Array.isArray(ids) ? ids[0] : ids)
        }
      }).then(() => {
        this.getList()
