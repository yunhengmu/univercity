  ...
  methods: {
    ...
    loadNoticeTop() {
      ...
      listNoticeTop().then(res => {
        ...
        // 验证数据，过滤掉 noticeId 无效的项
        this.noticeList = dataList.filter(item => {
          if (!item.noticeId || isNaN(Number(item.noticeId))) {
            console.warn('发现无效的公告数据:', item)
            return false
          }
          return true
        })
    ...
    previewNotice(item) {
      // 验证 noticeId 是否为有效数字
      if (!item.noticeId || isNaN(Number(item.noticeId))) {
        console.error('无效的公告ID:', item.noticeId)
        this.$message.error('公告数据异常，请刷新页面')
        return
      }
